...
public class solution {
	...
	public static int myAtoi(String str) {
		...
		final int INT_MIN = (int) (Math.pow(2, 31) * -1) ;
		...
		for ( int i = 0;i < sbuilder.length();i++) {
			...
			try  {
				...
				if (sbuilder.substring(i,i+1).equals("-") ) {
					ret.append(sbuilder.substring(i,i+1));
					
					
				}
				
				else if (sbuilder.substring(i,i+1).equals(" ")) {
					continue;
				}
				
				else {
					
					ret.append( Integer.parseInt( sbuilder.substring(i,i+1) ) );
				}
